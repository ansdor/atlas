...
pub fn query(args: &interface::QueryArguments, log: &mut Option<impl Write>) -> utils::GeneralResult<()> {
    let mut results = Vec::new();
    let pack_arguments_list = generate_mock_packing_arguments(&args);
    for p in pack_arguments_list.into_iter() {
        let mut sink = if log.is_some() { Some(std::io::sink()) } else { None };
    ...
    report += format!("\n{0:<40}{1:>16}{2:>16}\n", "SETTINGS", "SIZE", "EFFICIENCY").as_str();
    ...
    report += format!("{}\n", "-".repeat(ruler_width)).as_str();
    report += format!("for the most efficient packing of these sources, use this command:\n").as_str();
    report += format!("\tatlas pack {}[sources] [output]\n\n", describe_args(&results[0].0)).as_str();
    report += "if texture rotation is not allowed, use this command:\n";
    report += format!(
        "\tatlas pack {}[sources] [output]\n",
        describe_args(&results.iter().filter(|x| {
            !x.0.rotate
        }).next().unwrap().0)
    )
    .as_str();
    report += format!("{}", "-".repeat(ruler_width)).as_str();
...
fn generate_mock_packing_arguments(query_args: &interface::QueryArguments) -> Vec<interface::PackArguments> {
