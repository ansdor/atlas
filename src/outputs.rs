...
pub fn prepare_output_directory(
    path: &str, path_type: PathType, log: &mut Option<impl Write>,
    ...
    match dir {
        x if x.is_file() => Err(format!(
            "invalid output directory: a file named '{}' already exists.",
            dir.display()
        )
        .into()),
        x if !x.exists() => {
            ...
            utils::info_message(
                log,
                format!("directory '{}' created successfully.", dir.display()),
            );
...
pub fn notify_overwrite<P: AsRef<Path>>(
    path: P, overwrite_allowed: bool,
    ...
    match (p.exists(), overwrite_allowed) {
        ...
        (_, false) => Err(format!(
            "file '{}' already exists. use the -o flag to enable overwriting.",
            p.display()
        )
        .into()),
    }
...
pub fn count_overwrites<P: AsRef<Path>>(paths: &[P]) -> u32 {
    paths
        .iter()
        .map(|p| if p.as_ref().exists() { 1 } else { 0 })
        .sum()
}
